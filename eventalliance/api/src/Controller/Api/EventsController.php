...
class EventsController extends ApiController
{
    ...
    private function _getUsersTeamsEvents($userId)
    {
        ...
        if (empty($checkSuperAdmin)) {
            ...
            for ($i = 0; $i < count($rsEventTeamMember); $i++) {
               array_push($arrAccessableEvent, $rsEventTeamMember[$i]['event_id']);
            }

            if(empty($arrAccessableEvent)){
                array_push($arrAccessableEvent, 0);
            }
            $eventsWhere['Events.id IN'] = $arrAccessableEvent;
        }
    ...
    public function addArtist()
    {
        ...
        if (!empty($arrEvent) || !empty($requestData["eventId"])) {
            if (count($arrEvent) > 1 && empty($requestData["eventId"])) {
                ...
                $eventId = !empty($requestData["eventId"]) ? $requestData["eventId"] : $arrEvent[0];
                ...
                if (empty($existingRecord)) {
                    ...
                    if (empty($eventArtistEntity->errors())) {
                        ...
                        $this->set([
                            "status"     => false,
                            "id"         => 0,
                            "message"    => "There was an error during database transaction".print_r($eventArtistEntity->errors(), true),
                            '_serialize' => ['status', 'id', 'message'],
                        ]);
                    }
                } else {
    ...
    public function getEvents($requestData = null, $length = 0, $start = 1)
    {
        ...
        $query = $tblEvents->find('all')
            ->contain(["SurveyQuestions" => ["SurveyQuestionOptions"], "EventDemographics", "EventGenres", "Budgets", "EventTeamMembers" => ["Users"], "Teams" => ["TeamMembers" => ["Users" => function($q){
    ...
    public function getTeamMemberVote()
    {
        ...
        for ($p = 0; $p < count($rsEventDetails["event_artists"]); $p++) {
            ...
            for ($i = 0; $i < count($rsTeamMembers); $i++) {
                ...
                $vote["photo"] = $rsTeamMembers[$i]["user"]["profile_pic"];
    ...
    private function _getEventServeyResults($isPickedOnly)
    {
        ...
        $query          = $tblTeamMembers->find()
            ...
            }, "TeamSurveys" => function ($q) use ($eventId) {
        ...
        for ($i = 0; $i < count($rsEventDetails["event_artists"]); $i++) {
            ...
            if(!empty($rsEventDetails["event_artists"][$i]["artist"]["artist_channels"][0])) {
              $channelSubscriberCount =$rsEventDetails["event_artists"][$i]["artist"]["artist_channels"][0]["channel"]["channel_subscriber_count"];
              $channelViewCount =$rsEventDetails["event_artists"][$i]["artist"]["artist_channels"][0]["channel"]["channel_view_count"];
             }else{
            ...
            for ($teamSurveyArtistVotesCounter = 0; $teamSurveyArtistVotesCounter < count($rsTeamSurveyAtistVotes); $teamSurveyArtistVotesCounter++) {
                if ($rsTeamSurveyAtistVotes[$teamSurveyArtistVotesCounter]["artist_id"] == $rsEventDetails["event_artists"][$i]["artist_id"]) {
                    ...
                    $teamTotalVotes = $rsTeamSurveyAtistVotes[$teamSurveyArtistVotesCounter]["total"];
                    break;
                }
            }
    ...
    function changeArtistStatus(){
        ...
        $tblEventArtists = TableRegistry::get("EventArtists");
        $rsEventArtists = $tblEventArtists->find()->where(["event_id" => $eventId, "artist_id" => $artistId])->hydrate(false)->toArray();

        if(empty($rsEventArtists)){
            $this->set([
                "status"     => false,
                "id"         => 0,
                "message"    => "Could not find a matching record with the specified event and artist",
                '_serialize' => ['status', 'id', 'message'],
            ]);
        } else {
            $tblEventArtists->updateAll(["quote_status" => $status], ["id" => $rsEventArtists[0]["id"]]);
            $this->set([
                "status"     => true,
                "id"         => $rsEventArtists[0]["id"],
                "message"    => "Artist's status has been changed Successfully",
                '_serialize' => ['status', 'id', 'message'],
            ]);
        }
    }
