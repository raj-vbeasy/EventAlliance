...
class ArtistsController extends ApiController
{
    ...
    public function save($artistData)
    {
        ...
        if (empty($id)) {
            ...
            if (!empty($artistData["channelIds"])) {
                ...
                if (count($rsExistingChannels) >= 1) {
                    if ($rsExistingChannels[0]["channel"][0]["channel_ids"] != $artistData["channelIds"]) {
        ...
        if (!empty($arrChannelResult['items'][0])) {
            ...
            if (empty($artistEntity->errors())) {
                ...
                if (empty($id)) {
                    if (!empty($artistData["channelIds"])) {
                        ...
                        $youTubeChannelData = $arrChannelResult['items'][0]['statistics'];
